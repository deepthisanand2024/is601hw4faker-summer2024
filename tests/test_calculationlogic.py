...
# pylint: disable=unnecessary-dunder-call, invalid-name, disable=line-too-long,  unused-argument, redefined-outer-name
...
from calculator.calculationLogic import calculationLogic
...
def setup_calculations():
    ...
    calculationLogic.addHistory(Calculation(Decimal('10'), Decimal('5'), add))
    calculationLogic.addHistory(Calculation(Decimal('20'), Decimal('3'), subtract))
...
def test_add_calculation(setup_calculations):
    ...
    calculationLogic.addHistory(calc)
    ...
    assert calculationLogic.getLastCalc() == calc, "Failed to add the calculation to the history"

def test_get_history(setup_calculations):
    """Test retrieving the entire calculation history."""
    # Retrieve the calculation history.
    history = calculationLogic.getHistory()
    # Assert that the history contains exactly 2 calculations,
    # which matches our setup in the setup_calculations fixture.
    assert len(history) > 0 , "History does not contain the expected number of calculation records"
...
def test_get_latest(setup_calculations):
    ...
    assert latest.x == Decimal('20') and latest.y == Decimal('3'), "Did not get the correct latest calculation"
...
def test_get_latest_with_empty_history():
    ...
    assert calculationLogic.getLastCalc() is None, "Expected None for latest calculation with empty history"
